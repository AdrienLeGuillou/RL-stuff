...
def get_transition_rewards_probs(env):
    ...
    # poisson probs (location a or b), (pmf(r0) or sf(r0-1), r0)
    # goes to env.max_car + 1 for it can be needed later
    # sf(r0 -1) is the probability of getting r0 or more
    p_retu = np.zeros((2, 2, env.max_car + 2))
    p_rent = np.zeros((2, 2, env.max_car + 2))
    # skellam probs
    p_ske = np.zeros(((2, 2, env.max_car + 2)))


    # populate the poisson probs
    for i in range(2):
        pois_retu = poisson(env.mean_ret[i])
        pois_rent = poisson(env.mean_rent[i])
        skell = skellam(env.mean_ret[i], env.mean_rent[i])
        for j in range(env.max_car + 2):
            p_retu[i, 0, j] = pois_retu.pmf(j)
            p_retu[i, 1, j] = pois_retu.sf(j-1)
            p_rent[i, 0, j] = pois_rent.pmf(j)
            p_rent[i, 1, j] = pois_rent.sf(j-1)

    def make_row(i, env):
        # same as the full one but with one less dimension
        rj_sas = np.zeros((env.max_car + 1,
                          env.max_move * 2 + 1,
                          env.max_car + 1, env.max_car + 1))
        pj_sas = np.zeros((env.max_car + 1,
                          env.max_move * 2 + 1,
                          env.max_car + 1, env.max_car + 1))

        for j in range(env.max_car + 1):
            env.observation = [i, j]
            ...
            for m in moves:
                ...
                for r0, r1 in np.ndindex((i_ + 1, j_ + 1)):
                    ...
                    for k, l in np.ndindex((env.max_car - i__ + 1,
                                            env.max_car - j__ + 1)):
                        prob__ = 1
                        # same here. Returns get the nb of cars to env.max_car
                        # for all values > env.max_car - i__. Hence cdf
                        ...
                        a =  m + env.max_move
                        # beware a s_ can be explored several times
                        rj_sas[j, a, i__ + k, j__ + l] += (reward + rents) \
                                                        * prob_

                        pj_sas[j, a, i__ + k, j__ + l] +=  prob_ * prob__

        return rj_sas, pj_sas

    for i in range(env.max_car + 1):
        res = make_row(i, env)
        r_sas[i] = res[0]
        p_sas[i] = res[1]
